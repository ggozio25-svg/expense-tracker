...
function openVeicoloModal(id = null) {
    ...
    document.getElementById('veicolo-anno').value = new Date().getFullYear();
    ...
    if (id) {
        ...
        if (veicolo) {
            ...
            document.getElementById('veicolo-anno').value = veicolo.anno || new Date().getFullYear();
...
async function saveVeicolo(e) {
    ...
    try {
        ...
        const data = {
            ...
            anno: parseInt(document.getElementById('veicolo-anno').value) || new Date().getFullYear(),
    ...
    try {
        ...
        if (result.success && result.ocr_data) {
            ...
            if (result.ocr_data.data) {
                ...
                if (dateMatch) {
                    const [, giorno, mese, annoData] = dateMatch;
                    document.getElementById('spesa-data').value = `${annoData}-${mese}-${giorno}`;
                }
            }
...
async function exportSpese() {
    ...
    try {
        ...
        const dataFine = document.getElementById('export-spese-a').value;
        ...
        const response = await fetch(`${API_URL}/export/excel`, {
            ...
            body: JSON.stringify({
                tipo: 'spese',
                filtri: { data_inizio: da, data_fine: dataFine, cliente_id: cliente }
            })
        });
...
async function exportKm() {
    ...
    try {
        ...
        const dataFine = document.getElementById('export-km-a').value;
        ...
        const response = await fetch(`${API_URL}/export/excel`, {
            ...
            body: JSON.stringify({
                tipo: 'chilometriche',
                filtri: { data_inizio: da, data_fine: dataFine }
            })
        });
